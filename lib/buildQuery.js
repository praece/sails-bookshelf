...
function applyQuery(query, params, includeTable, Model) {
  ...
  if (params.flattenWhere || _.isEmpty(params.where)) return;
...
function getNestedValue(clause) {
  if (_.isPlainObject(clause.value)) {
    ...
    if (_.isArray(clause.value) && !clause.operator) clause.operator = 'in';
...
function applyClause(clause, or) {
  ...
  if (operator === '?|') return query.whereRaw('?? \\?| array[' + _.join(_.times(value.length, _.constant('?')), ', ') + ']', _.concat([table + '.' + field], value));
