...
function applyQuery(query, params, includeTable, Model) {
  ...
  const where = _.map(params.where, buildWhere.bind(bind));

  _.forEach(where, applyClause.bind(bind));
};
...
function applyJoin(relations) {
  ...
  if (!this.joins.length) {
    var raw = sails.db.knex.raw;

    // On the first join make sure we don't return duplicate results
    this.query.select(raw('distinct on (??) ??', [`${parentTable}.id`, `${parentTable}.*`]));
  }
