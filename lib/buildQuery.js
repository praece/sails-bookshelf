...
function applyQuery(query, params, includeTable, Model) {
  var model = new Model();
  const whereQuery = query.clone();
  const bind = { model, query, joins: [], Model };
  const whereBind = _.assign({}, bind, { query: whereQuery });
  ...
  whereQuery.select(`${model.tableName}.id`);
  _.forEach(where, applyClause.bind(whereBind));

  query.whereIn(`${model.tableName}.id`, whereQuery);
};
