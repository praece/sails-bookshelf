...
function cacheFields(model, response, options) {
    ...
    .then(() => {
      ...
      return Promise.resolve(model.cacheFields(model, response, options))
        .then(fields => {
          ...
          if (model.set(fields).hasChanged()) return model.sync(options).update(fields);
        });
    })
    .then(() => {
      ...
      return Promise.mapSeries(model.triggerCachedFields, field => {
        ...
        return Promise.resolve(model)
          ...
          .then(related => {
            ...
            return cacheFields(related, related.toJSON(), opts);
          });
      });
    });
}
