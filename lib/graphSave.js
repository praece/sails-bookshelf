...
function setup(model, options) {
  const allowNestedSave = model.constructor.allowNestedSave;
  ...
  const graphing = _.map(model.relationships, function(relationship, key) {
    ...
    if (_.isArray(data) || _.isPlainObject(data)) {
      // If there is an object here but we don't allow saving just wipe it and return
      if (allowNestedSave && !_.includes(allowNestedSave, key)) {
        model.unset(key);
        return;
      }
