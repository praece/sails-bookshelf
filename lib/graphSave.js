...
var omitOptions = ['query', 'previousAttributes', 'method', 'graphData', 'changed', 'files'];
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(function() {
      ...
      _.forEach(options.graphData, function(data, key) {
        ...
        if (_.includes(['belongsTo', 'morphTo'], relation.relatedData.type)) {
          var nestOptions = _.assign({ nested: {} }, _.omit(options, omitOptions));
          nestOptions.nested[model.tableName] = { method: options.method, data: model.toJSON() };
  ...
  _.forEach(options.graphData, function(data, key) {
    ...
    if (_.includes(['morphMany', 'hasMany', 'belongsToMany'], relation.relatedData.type)) {
      _.forEach(data, function(item) {
        ...
        const promise = safeRelation.create(item, nestOptions)
          .then(output => {
            relation.add(output);
          });
